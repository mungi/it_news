...
## 2026-07-11 로컬 에이전트·공급망 경계 추가
- Mac mini/Studio를 별도 AI 에이전트 실행 장비로 쓰는 수요는 endpoint가 agent runtime node로 바뀌는 흐름을 의미함. MDM, secret isolation, sandbox, audit log 기준 필요.
- Injective Labs GitHub compromise와 npm wallet-key-stealing packages 사례는 GitHub 조직 보안과 registry publish 권한을 하나의 supply-chain incident runbook으로 묶어야 함을 확인.

## 2026-07-11 업데이트: 에이전트·하드웨어 IP 경계
- Apple-OpenAI 영업비밀 소송은 AI 하드웨어 조직의 인력 이동, prototype 문서, 공급망 정보가 agent/device 경쟁의 보안 경계가 됨을 보여줌.
- fenic 사례는 에이전트가 호출하는 데이터 파이프라인도 typed schema, lineage, token/cost metric, MCP 권한 모델을 갖춘 통제 대상임을 보여줌.

## 2026-07-11 refresh: 범용 업무 에이전트 권한 경계
- Cursor Sand 보도는 코딩 에이전트 기업이 이메일, 문서, 일정, 스프레드시트까지 연결된 범용 업무 에이전트로 확장하는 흐름을 보여줌.
- 보안 기준은 IDE sandbox만으로 부족함. workspace connector 최소 권한, destructive action 승인, DLP, cross-app audit, offboarding runbook을 제품 평가 항목에 포함 필요.

## 2026-07-11 evening refresh: 이미지·첨부 파일 프롬프트 주입
- Ghostcommit 보도는 이미지 파일 안의 숨은 지시가 AI code review와 coding agent 사이의 가시성 차이를 악용할 수 있음을 보여줌.
- 방어 기준은 텍스트 diff 검토만으로 부족함. 이미지 OCR/metadata 검사, binary/document file gate, secret path denylist, output secret scanning, tool-call file access audit을 PR workflow에 포함 필요.

## 2026-07-12 refresh: npm install-time credential theft
- jscrambler 8.14.0 침해는 `preinstall` hook이 CI·개발자 장비의 cloud credential, AI coding tool 설정, MCP credential을 한 번에 겨냥할 수 있음을 보여줌.
- 방어 기준은 npm provenance, install script allowlist, CI short-lived credential, package diff gate, AI tool config secret scanning을 하나의 supply-chain runbook으로 통합 필요.

## 2026-07-12 late-morning refresh: 조직화된 AI abuse 대응
- CASP의 Boko Haram 전직 구성원 인터뷰는 frontier AI 악용이 단일 prompt가 아니라 계정·구독·VPN·교육·우회 지식 전파가 결합된 조직 운영 문제임을 보여줌.
- AI provider와 enterprise API 조달팀은 abuse monitoring을 prompt 필터, 계정 군집, 결제·네트워크 패턴, cross-model 재시도, high-risk escalation SLA까지 확장 필요.

## 2026-07-13 refresh: 모바일 단말 권한 경계
- RedHook의 Wireless ADB 악용은 Android 단말에서 Accessibility 권한, Developer Options, Wireless Debugging이 결합될 때 루팅 없이 shell UID 2000 권한까지 이어질 수 있음을 보여줌.
- 모바일 보안팀은 AI/업무 앱 보호와 별개로 MDM에서 무선 디버깅 차단, 접근성 권한 신규 승인 감시, Shizuku류 권한 브리지 탐지, fake Play 설치 유도 차단을 정책화 필요.

## 2026-07-14 refresh: 장기 메모리 오염 경계
- MemGhost 연구는 inbox를 읽는 agent가 외부 이메일의 지시를 AGENTS.md·MEMORY.md·vector store 같은 persistent state에 기록하면, 단발성 prompt injection이 다음 세션까지 이어지는 memory poisoning으로 바뀜을 실험함.
- 방어 기준은 입력 필터만이 아니라 external-content provenance, memory write allowlist, user/admin approval, immutable diff audit, TTL·snapshot rollback, background agent의 capability 축소를 함께 적용하는 구조임.
