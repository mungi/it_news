...
4. Deep Dive count must be 1 by default and never more than 2. When 2 Deep Dives exist, refresh every selected Deep Dive topic once per KST calendar day; do not leave any yesterday's topic in place by default.
...
- Deep Dive daily-refresh policy: on the first update run of each KST date, research and replace every existing Deep Dive selection with the strongest current candidates from the coverage window. If 2 entries exist, select two distinct story/events with non-overlapping primary sources. Prefer a different story/event and at least one fresh primary/credible source published or materially updated since each prior selection. Rebuild each title, summary, details, `detailed_content`, sources, and image metadata from the selected sources; never merely change timestamps.
- Add `refreshed_kst` to every selected Deep Dive with the actual refresh timestamp. Keep an entry's topic only when no credible replacement exists; in that case update it only with a verifiable material source change and add `refresh_note` explaining why the topic remained. Do not fabricate a daily change.
- Before writing, compare each prior Deep Dive `sources`, title, and `refreshed_kst` against its candidate. A repeated source URL or unchanged topic on a new KST date requires the documented no-replacement exception above.
